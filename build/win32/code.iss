...
AppPublisher=bring.out doo Sarajevo
...
Name: "{userappdata}\bringout\Internet Explorer\Quick Launch\{#NameLong}"; Filename: "{app}\{#ExeBasename}.exe"; Tasks: quicklaunchicon; AppUserModelID: "{#AppUserId}"
...
begin
  Result := True;
    ...
    if Result and not WizardSilent() then begin
      RegKey := 'SOFTWARE\bringout\Windows\CurrentVersion\Uninstall\' + copy('{#IncompatibleTargetAppId}', 2, 38) + '_is1';
  ...
  if Result and IsWin64 then begin
    RegKey := 'SOFTWARE\bringout\Windows\CurrentVersion\Uninstall\' + copy('{#IncompatibleArchAppId}', 2, 38) + '_is1';
